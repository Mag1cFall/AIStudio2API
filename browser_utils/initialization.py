...
async def _add_init_scripts_to_context(context: AsyncBrowserContext):
    ...
    try:
        ...
        # 2. 注入用于 Enter 提交和图片粘贴的新脚本
        interaction_enhancements_js = """
        // AISTudio Interaction Enhancements (Injected by Python)
        (function() {
            'use strict';
            const LOG_PREFIX = '[AI Studio Enhancements]';
            function log(...args) { console.log(LOG_PREFIX, ...args); }
            function error(...args) { console.error(LOG_PREFIX, ...args); }
            function warn(...args) { console.warn(LOG_PREFIX, ...args); }

            function setupInteractionEnhancements() {
                const PROMPT_TEXTAREA_SELECTOR = 'ms-prompt-input-wrapper ms-autosize-textarea textarea';
                const SUBMIT_BUTTON_SELECTOR = 'button[aria-label="Run"].run-button';
                const FILE_INPUT_SELECTOR = 'input[type="file"]';

                const observer = new MutationObserver((mutations, obs) => {
                    const textarea = document.querySelector(PROMPT_TEXTAREA_SELECTOR);
                    if (textarea) {
                        log('Chat textarea found, attaching event listeners...');
                        attachEventListeners(textarea);
                        obs.disconnect(); // Stop observing once the element is found and listeners are attached
                    }
                });
                    ...
                    textarea.addEventListener('keydown', (event) => {
                        if ((event.key === 'Enter' && !event.shiftKey) || (event.key === 'Enter' && event.ctrlKey)) {
                            ...
                            if (submitButton && !submitButton.disabled) {
                                log('Submitting with Enter or Ctrl+Enter...');
                                submitButton.click();
                            } else {
                                warn('Submit button not found or is disabled.');
                            }
                        }
                    });
                        ...
                        if (files.length > 0 && files[0].type.startsWith('image/')) {
                            ...
                            if (fileInput) {
                                try {
                                    ...
                                    log('Pasted image assigned to file input and change event dispatched.');
                                } catch (e) {
                                    error('Error while assigning pasted file to input:', e);
                                }
                            } else {
                                warn('Could not find a file input element for upload. Paste-to-upload failed.');
                            }
                        }
                    });
                    log('Interaction event listeners attached.');
                }
            }
        ...
        """
        await context.add_init_script(interaction_enhancements_js)
        logger.info('✅ 已将交互增强脚本添加到浏览器上下文。')
    except Exception as e:
