...
from browser_utils import switch_ai_studio_model, save_error_snapshot, _wait_for_response_completion, _get_final_response_content, detect_and_extract_page_error
from .utils import validate_chat_request, prepare_combined_prompt, generate_sse_chunk, generate_sse_stop_chunk, generate_sse_error_chunk, use_helper_get_response, use_stream_response
...
async def _process_request_refactored(req_id: str, request: ChatCompletionRequest, http_request: Request, result_future: Future) -> Optional[Tuple[Event, Locator, Callable[[str], bool]]]:
    ...
    try:
                ...
                if current_ai_studio_model_id != model_id_to_use:
                    ...
                    if switch_success:
                        ...
                        try:
                            ...
                            pass
                        raise HTTPException(status_code=422, detail=f"[{req_id}] AI Studio 未能应用所请求的模型 '{model_id_to_use}' 或该模型不受支持。请选择 AI Studio 网页界面中可用的模型。当前实际生效的模型 ID 为 '{current_ai_studio_model_id}', 页面显示为 '{actual_displayed_model_name}'.")
                else:
        ...
        async with params_cache_lock:
            ...
            if model_actually_switched_in_current_api_call or (current_ai_studio_model_id is not None and current_ai_studio_model_id != cached_model_for_params):
        ...
        if is_streaming:
            ...
            response_payload = {'id': f'{CHAT_COMPLETION_ID_PREFIX}{req_id}-{int(time.time())}', 'object': 'chat.completion', 'created': int(time.time()), 'model': MODEL_NAME, 'choices': [{'index': 0, 'message': {'role': 'assistant', 'content': '处理完成 - 需要完整逻辑'}, 'finish_reason': 'stop'}], 'usage': {'prompt_tokens': 0, 'completion_tokens': 0, 'total_tokens': 0}}
            if not result_future.done():
