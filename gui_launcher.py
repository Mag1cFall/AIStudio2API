...
AUTH_PROFILES_DIR = os.path.join(SCRIPT_DIR, 'auth_profiles')
ACTIVE_AUTH_DIR = os.path.join(AUTH_PROFILES_DIR, 'active')
SAVED_AUTH_DIR = os.path.join(AUTH_PROFILES_DIR, 'saved')
DEFAULT_FASTAPI_PORT = int(os.environ.get('DEFAULT_FASTAPI_PORT', '2048'))
DEFAULT_CAMOUFOX_PORT_GUI = int(os.environ.get('DEFAULT_CAMOUFOX_PORT', '9222'))
managed_process_info: Dict[str, Any] = {'popen': None, 'service_name_key': None, 'monitor_thread': None, 'stdout_thread': None, 'stderr_thread': None, 'output_area': None, 'fully_detached': False}
...
LANG_TEXTS = {'title': {'zh': 'AI Studio Proxy API Launcher GUI', 'en': 'AI Studio Proxy API Launcher GUI'}, 'status_idle': {'zh': '空闲，请选择操作。', 'en': 'Idle. Select an action.'}, 'port_section_label': {'zh': '服务端口配置', 'en': 'Service Port Configuration'}, 'port_input_description_lbl': {'zh': '提示: 启动时将使用下方指定的FastAPI服务端口和Camoufox调试端口。', 'en': 'Note: The FastAPI service port and Camoufox debug port specified below will be used for launch.'}, 'fastapi_port_label': {'zh': 'FastAPI 服务端口:', 'en': 'FastAPI Port:'}, 'camoufox_debug_port_label': {'zh': 'Camoufox 调试端口:', 'en': 'Camoufox Debug Port:'}, 'query_pids_btn': {'zh': '查询端口进程', 'en': 'Query Port Processes'}, 'stop_selected_pid_btn': {'zh': '停止选中进程', 'en': 'Stop Selected Process'}, 'pids_on_port_label': {'zh': '端口占用情况 (PID - 名称):', 'en': 'Processes on Port (PID - Name):'}, 'pids_on_port_label_dynamic': {'zh': '端口 {port} 占用情况 (PID - 名称):', 'en': 'Processes on Port {port} (PID - Name):'}, 'no_pids_found': {'zh': '未找到占用该端口的进程。', 'en': 'No processes found on this port.'}, 'static_pid_list_title': {'zh': '启动所需端口占用情况 (PID - 名称)', 'en': 'Required Ports Usage (PID - Name)'}, 'launch_options_label': {'zh': '启动选项', 'en': 'Launch Options'}, 'launch_options_note_revised': {'zh': '提示：有头/无头模式均会在新的独立终端窗口中启动服务。\n有头模式用于调试和认证。无头模式需预先认证。\n关闭此GUI不会停止已独立启动的服务。', 'en': 'Tip: Headed/Headless modes will launch the service in a new independent terminal window.\nHeaded mode is for debug and auth. Headless mode requires pre-auth.\nClosing this GUI will NOT stop independently launched services.'}, 'launch_headed_interactive_btn': {'zh': '启动有头模式 (新终端)', 'en': 'Launch Headed Mode (New Terminal)'}, 'launch_headless_btn': {'zh': '启动无头模式 (新终端)', 'en': 'Launch Headless Mode (New Terminal)'}, 'launch_virtual_display_btn': {'zh': '启动虚拟显示模式 (Linux)', 'en': 'Launch Virtual Display (Linux)'}, 'open_webui_btn': {'zh': '打开 WebUI 控制台', 'en': 'Open WebUI Dashboard'}, 'stop_gui_service_btn': {'zh': '停止当前GUI管理的服务', 'en': 'Stop Current GUI-Managed Service'}, 'status_label': {'zh': '状态', 'en': 'Status'}, 'output_label': {'zh': '输出日志', 'en': 'Output Log'}, 'menu_language_fixed': {'zh': 'Language', 'en': 'Language'}, 'menu_lang_zh_option': {'zh': '中文 (Chinese)', 'en': '中文 (Chinese)'}, 'menu_lang_en_option': {'zh': '英文 (English)', 'en': '英文 (English)'}, 'confirm_quit_title': {'zh': '确认退出', 'en': 'Confirm Quit'}, 'confirm_quit_message': {'zh': '服务可能仍在独立终端中运行。确认退出GUI吗?', 'en': 'Services may still be running in independent terminals. Confirm quit GUI?'}, 'confirm_quit_message_independent': {'zh': "独立后台服务 '{service_name}' 可能仍在运行。直接退出GUI吗 (服务将继续运行)?", 'en': "Independent background service '{service_name}' may still be running. Quit GUI (service will continue to run)?"}, 'error_title': {'zh': '错误', 'en': 'Error'}, 'info_title': {'zh': '信息', 'en': 'Info'}, 'warning_title': {'zh': '警告', 'en': 'Warning'}, 'service_already_running': {'zh': '服务 ({service_name}) 已在运行。', 'en': 'A service ({service_name}) is already running.'}, 'proxy_config_title': {'zh': '代理配置', 'en': 'Proxy Configuration'}, 'proxy_config_message_generic': {'zh': '是否为此启动启用 HTTP/HTTPS 代理?', 'en': 'Enable HTTP/HTTPS proxy for this launch?'}, 'proxy_address_title': {'zh': '代理地址', 'en': 'Proxy Address'}, 'proxy_address_prompt': {'zh': '输入代理地址 (例如 http://host:port)\n默认: {default_proxy}', 'en': 'Enter proxy address (e.g., http://host:port)\nDefault: {default_proxy}'}, 'proxy_configured_status': {'zh': '代理已配置: {proxy_addr}', 'en': 'Proxy configured: {proxy_addr}'}, 'proxy_skip_status': {'zh': '用户跳过代理设置。', 'en': 'Proxy setup skipped by user.'}, 'script_not_found_error_msgbox': {'zh': '启动失败: 未找到 Python 执行文件或脚本。\n命令: {cmd}', 'en': 'Failed to start: Python executable or script not found.\nCommand: {cmd}'}, 'startup_error_title': {'zh': '启动错误', 'en': 'Startup Error'}, 'startup_script_not_found_msgbox': {'zh': "必需的脚本 '{script}' 在当前目录未找到。\n请将此GUI启动器与 launch_camoufox.py 和 server.py 放在同一目录。", 'en': "Required script '{script}' not found in the current directory.\nPlace this GUI launcher in the same directory as launch_camoufox.py and server.py."}, 'service_starting_status': {'zh': '{service_name} 启动中... PID: {pid}', 'en': '{service_name} starting... PID: {pid}'}, 'service_stopped_gracefully_status': {'zh': '{service_name} 已平稳停止。', 'en': '{service_name} stopped gracefully.'}, 'service_stopped_exit_code_status': {'zh': '{service_name} 已停止。退出码: {code}', 'en': '{service_name} stopped. Exit code: {code}'}, 'service_stop_fail_status': {'zh': '{service_name} (PID: {pid}) 未能平稳终止。正在强制停止...', 'en': '{service_name} (PID: {pid}) did not terminate gracefully. Forcing kill...'}, 'service_killed_status': {'zh': '{service_name} (PID: {pid}) 已被强制停止。', 'en': '{service_name} (PID: {pid}) killed.'}, 'error_stopping_service_msgbox': {'zh': '停止 {service_name} (PID: {pid}) 时出错: {e}', 'en': 'Error stopping {service_name} (PID: {pid}): {e}'}, 'no_service_running_status': {'zh': '当前没有GUI管理的服务在运行。', 'en': 'No GUI-managed service is currently running.'}, 'stopping_initiated_status': {'zh': '{service_name} (PID: {pid}) 停止已启动。最终状态待定。', 'en': '{service_name} (PID: {pid}) stopping initiated. Final status pending.'}, 'service_name_headed_interactive': {'zh': '有头交互服务', 'en': 'Headed Interactive Service'}, 'service_name_headless': {'zh': '无头服务', 'en': 'Headless Service'}, 'service_name_virtual_display': {'zh': '虚拟显示无头服务', 'en': 'Virtual Display Headless Service'}, 'status_headed_launch': {'zh': '有头模式：启动中，请关注新控制台的提示...', 'en': 'Headed Mode: Launching, check new console for prompts...'}, 'status_headless_launch': {'zh': '无头服务：启动中...新的独立终端将打开。', 'en': 'Headless Service: Launching... A new independent terminal will open.'}, 'status_virtual_display_launch': {'zh': '虚拟显示模式启动中...', 'en': 'Virtual Display Mode launching...'}, 'info_service_is_independent': {'zh': '当前服务为独立后台进程，关闭GUI不会停止它。请使用系统工具或端口管理手动停止此服务。', 'en': 'The current service is an independent background process. Closing the GUI will not stop it. Please manage this service manually using system tools or port management.'}, 'info_service_new_terminal': {'zh': '服务已在新的独立终端启动。关闭此GUI不会影响该服务。', 'en': 'Service has been launched in a new independent terminal. Closing this GUI will not affect the service.'}, 'warn_cannot_stop_independent_service': {'zh': '通过此GUI启动的服务在独立终端中运行，无法通过此按钮停止。请直接管理其终端或使用系统工具。', 'en': 'Services launched via this GUI run in independent terminals and cannot be stopped by this button. Please manage their terminals directly or use system tools.'}, 'enter_valid_port_warn': {'zh': '请输入有效的端口号 (1024-65535)。', 'en': 'Please enter a valid port number (1024-65535).'}, 'pid_list_empty_for_stop_warn': {'zh': '进程列表为空或未选择进程。', 'en': 'PID list is empty or no process selected.'}, 'confirm_stop_pid_title': {'zh': '确认停止进程', 'en': 'Confirm Stop Process'}, 'confirm_stop_pid_message': {'zh': '确定要尝试停止 PID {pid} ({name}) 吗?', 'en': 'Are you sure you want to attempt to stop PID {pid} ({name})?'}, 'confirm_stop_pid_admin_title': {'zh': '以管理员权限停止进程', 'en': 'Stop Process with Admin Privileges'}, 'confirm_stop_pid_admin_message': {'zh': '以普通权限停止 PID {pid} ({name}) 可能失败。是否尝试使用管理员权限停止?', 'en': 'Stopping PID {pid} ({name}) with normal privileges may fail. Try with admin privileges?'}, 'admin_stop_success': {'zh': '已成功使用管理员权限停止 PID {pid}', 'en': 'Successfully stopped PID {pid} with admin privileges'}, 'admin_stop_failure': {'zh': '使用管理员权限停止 PID {pid} 失败: {error}', 'en': 'Failed to stop PID {pid} with admin privileges: {error}'}, 'status_error_starting': {'zh': '启动 {service_name} 失败。', 'en': 'Error starting {service_name}'}, 'status_script_not_found': {'zh': '错误: 未找到 {service_name} 的可执行文件/脚本。', 'en': 'Error: Executable/script not found for {service_name}.'}, 'error_getting_process_name': {'zh': '获取 PID {pid} 的进程名失败。', 'en': 'Failed to get process name for PID {pid}.'}, 'pid_info_format': {'zh': 'PID: {pid} (端口: {port}) - 名称: {name}', 'en': 'PID: {pid} (Port: {port}) - Name: {name}'}, 'status_stopping_service': {'zh': '正在停止 {service_name} (PID: {pid})...', 'en': 'Stopping {service_name} (PID: {pid})...'}, 'error_title_invalid_selection': {'zh': '无效的选择格式: {selection}', 'en': 'Invalid selection format: {selection}'}, 'error_parsing_pid': {'zh': "无法从 '{selection}' 解析PID。", 'en': "Could not parse PID from '{selection}'."}, 'terminate_request_sent': {'zh': '终止请求已发送。', 'en': 'Termination request sent.'}, 'terminate_attempt_failed': {'zh': '尝试终止 PID {pid} ({name}) 可能失败。', 'en': 'Attempt to terminate PID {pid} ({name}) may have failed.'}, 'unknown_process_name_placeholder': {'zh': '未知进程名', 'en': 'Unknown Process Name'}, 'kill_custom_pid_label': {'zh': '或输入PID终止:', 'en': 'Or Enter PID to Kill:'}, 'kill_custom_pid_btn': {'zh': '终止指定PID', 'en': 'Kill Specified PID'}, 'pid_input_empty_warn': {'zh': '请输入要终止的PID。', 'en': 'Please enter a PID to kill.'}, 'pid_input_invalid_warn': {'zh': '输入的PID无效，请输入纯数字。', 'en': 'Invalid PID entered. Please enter numbers only.'}, 'confirm_kill_custom_pid_title': {'zh': '确认终止PID', 'en': 'Confirm Kill PID'}, 'status_sending_sigint': {'zh': '正在向 {service_name} (PID: {pid}) 发送 SIGINT...', 'en': 'Sending SIGINT to {service_name} (PID: {pid})...'}, 'status_waiting_after_sigint': {'zh': '{service_name} (PID: {pid})：SIGINT 已发送，等待 {timeout} 秒优雅退出...', 'en': '{service_name} (PID: {pid}): SIGINT sent, waiting {timeout}s for graceful exit...'}, 'status_sigint_effective': {'zh': '{service_name} (PID: {pid}) 已响应 SIGINT 并停止。', 'en': '{service_name} (PID: {pid}) responded to SIGINT and stopped.'}, 'status_sending_sigterm': {'zh': '{service_name} (PID: {pid})：未在规定时间内响应 SIGINT，正在发送 SIGTERM...', 'en': '{service_name} (PID: {pid}): Did not respond to SIGINT in time, sending SIGTERM...'}, 'status_waiting_after_sigterm': {'zh': '{service_name} (PID: {pid})：SIGTERM 已发送，等待 {timeout} 秒优雅退出...', 'en': '{service_name} (PID: {pid}): SIGTERM sent, waiting {timeout}s for graceful exit...'}, 'status_sigterm_effective': {'zh': '{service_name} (PID: {pid}) 已响应 SIGTERM 并停止。', 'en': '{service_name} (PID: {pid}) responded to SIGTERM and stopped.'}, 'status_forcing_kill': {'zh': '{service_name} (PID: {pid})：未在规定时间内响应 SIGTERM，正在强制终止 (SIGKILL)...', 'en': '{service_name} (PID: {pid}): Did not respond to SIGTERM in time, forcing kill (SIGKILL)...'}, 'enable_stream_proxy_label': {'zh': '启用流式代理服务', 'en': 'Enable Stream Proxy Service'}, 'stream_proxy_port_label': {'zh': '流式代理端口:', 'en': 'Stream Proxy Port:'}, 'enable_helper_label': {'zh': '启用外部Helper服务', 'en': 'Enable External Helper Service'}, 'helper_endpoint_label': {'zh': 'Helper端点URL:', 'en': 'Helper Endpoint URL:'}, 'auth_manager_title': {'zh': '认证文件管理', 'en': 'Authentication File Manager'}, 'saved_auth_files_label': {'zh': '已保存的认证文件:', 'en': 'Saved Authentication Files:'}, 'no_file_selected': {'zh': '请选择一个认证文件', 'en': 'Please select an authentication file'}, 'auth_file_activated': {'zh': "认证文件 '{file}' 已成功激活", 'en': "Authentication file '{file}' has been activated successfully"}, 'error_activating_file': {'zh': "激活文件 '{file}' 时出错: {error}", 'en': "Error activating file '{file}': {error}"}, 'auth_deactivated': {'zh': '已取消激活所有认证', 'en': 'All authentications deactivated'}, 'error_deactivating': {'zh': '取消激活时出错: {error}', 'en': 'Error during deactivation: {error}'}, 'activate_selected_btn': {'zh': '激活选中的文件', 'en': 'Activate Selected File'}, 'deactivate_btn': {'zh': '取消激活', 'en': 'Deactivate'}, 'cancel_btn': {'zh': '取消', 'en': 'Cancel'}, 'auth_files_management': {'zh': '认证文件管理', 'en': 'Auth Files Management'}, 'manage_auth_files_btn': {'zh': '管理认证文件', 'en': 'Manage Auth Files'}, 'no_saved_auth_files': {'zh': '保存目录中没有认证文件', 'en': 'No authentication files in saved directory'}, 'auth_dirs_missing': {'zh': '认证目录不存在，请确保目录结构正确', 'en': 'Authentication directories missing, please ensure correct directory structure'}, 'confirm_kill_port_title': {'zh': '确认清理端口', 'en': 'Confirm Port Cleanup'}, 'confirm_kill_port_message': {'zh': '端口 {port} 被以下PID占用: {pids}。是否尝试终止这些进程?', 'en': 'Port {port} is in use by PID(s): {pids}. Try to terminate them?'}, 'port_cleared_success': {'zh': '端口 {port} 已成功清理', 'en': 'Port {port} has been cleared successfully'}, 'port_still_in_use': {'zh': '端口 {port} 仍被占用，请手动处理', 'en': 'Port {port} is still in use, please handle manually'}, 'port_in_use_no_pids': {'zh': '端口 {port} 被占用，但无法识别进程', 'en': 'Port {port} is in use, but processes cannot be identified'}, 'error_removing_file': {'zh': "删除文件 '{file}' 时出错: {error}", 'en': "Error removing file '{file}': {error}"}, 'stream_port_out_of_range': {'zh': '流式代理端口必须为0(禁用)或1024-65535之间的值', 'en': 'Stream proxy port must be 0 (disabled) or a value between 1024-65535'}, 'port_auto_check': {'zh': '启动前自动检查端口', 'en': 'Auto-check port before launch'}, 'auto_port_check_enabled': {'zh': '已启用端口自动检查', 'en': 'Port auto-check enabled'}, 'port_check_running': {'zh': '正在检查端口 {port}...', 'en': 'Checking port {port}...'}, 'port_name_fastapi': {'zh': 'FastAPI服务', 'en': 'FastAPI Service'}, 'port_name_camoufox_debug': {'zh': 'Camoufox调试', 'en': 'Camoufox Debug'}, 'port_name_stream_proxy': {'zh': '流式代理', 'en': 'Stream Proxy'}, 'checking_port_with_name': {'zh': '正在检查{port_name}端口 {port}...', 'en': 'Checking {port_name} port {port}...'}, 'port_check_all_completed': {'zh': '所有端口检查完成', 'en': 'All port checks completed'}, 'port_check_failed': {'zh': '{port_name}端口 {port} 检查失败，启动已中止', 'en': '{port_name} port {port} check failed, launch aborted'}, 'port_name_helper_service': {'zh': 'Helper服务', 'en': 'Helper Service'}, 'confirm_kill_multiple_ports_title': {'zh': '确认清理多个端口', 'en': 'Confirm Multiple Ports Cleanup'}, 'confirm_kill_multiple_ports_message': {'zh': '以下端口被占用:\n{occupied_ports_details}\n是否尝试终止这些进程?', 'en': 'The following ports are in use:\n{occupied_ports_details}\nAttempt to terminate these processes?'}, 'all_ports_cleared_success': {'zh': '所有选定端口已成功清理。', 'en': 'All selected ports have been cleared successfully.'}, 'some_ports_still_in_use': {'zh': '部分端口在清理后仍被占用，请手动处理。启动已中止。', 'en': 'Some ports are still in use after cleanup attempt. Please handle manually. Launch aborted.'}, 'port_check_user_declined_cleanup': {'zh': '用户选择不清理占用的端口，启动已中止。', 'en': 'User chose not to clean up occupied ports. Launch aborted.'}, 'reset_button': {'zh': '重置为默认设置', 'en': 'Reset to Defaults'}, 'confirm_reset_title': {'zh': '确认重置', 'en': 'Confirm Reset'}, 'confirm_reset_message': {'zh': '确定要重置所有设置为默认值吗？', 'en': 'Are you sure you want to reset all settings to default values?'}, 'reset_success': {'zh': '已重置为默认设置', 'en': 'Reset to default settings successfully'}, 'proxy_config_last_used': {'zh': '使用上次的代理: {proxy}', 'en': 'Using last proxy: {proxy}'}, 'proxy_config_other': {'zh': '使用其他代理地址', 'en': 'Use a different proxy address'}, 'service_closing_guide': {'zh': '关闭服务指南', 'en': 'Service Closing Guide'}, 'service_closing_guide_btn': {'zh': '如何关闭服务?', 'en': 'How to Close Service?'}, 'service_closing_guide_message': {'zh': service_closing_guide_message_zh, 'en': service_closing_guide_message_en}, 'enable_proxy_label': {'zh': '启用浏览器代理', 'en': 'Enable Browser Proxy'}, 'proxy_address_label': {'zh': '代理地址:', 'en': 'Proxy Address:'}, 'current_auth_file_display_label': {'zh': '当前认证: ', 'en': 'Current Auth: '}, 'current_auth_file_none': {'zh': '无', 'en': 'None'}, 'current_auth_file_selected_format': {'zh': '{file}', 'en': '{file}'}, 'test_proxy_btn': {'zh': '测试', 'en': 'Test'}, 'proxy_section_label': {'zh': '代理配置', 'en': 'Proxy Configuration'}, 'proxy_test_url_default': 'http://httpbin.org/get', 'proxy_test_url_backup': 'http://www.google.com', 'proxy_not_enabled_warn': {'zh': '代理未启用或地址为空，请先配置。', 'en': 'Proxy not enabled or address is empty. Please configure first.'}, 'proxy_test_success': {'zh': '代理连接成功 ({url})', 'en': 'Proxy connection successful ({url})'}, 'proxy_test_failure': {'zh': '代理连接失败 ({url}):\n{error}', 'en': 'Proxy connection failed ({url}):\n{error}'}, 'proxy_testing_status': {'zh': '正在测试代理 {proxy_addr}...', 'en': 'Testing proxy {proxy_addr}...'}, 'proxy_test_success_status': {'zh': '代理测试成功 ({url})', 'en': 'Proxy test successful ({url})'}, 'proxy_test_failure_status': {'zh': '代理测试失败: {error}', 'en': 'Proxy test failed: {error}'}, 'proxy_test_retrying': {'zh': '代理测试失败，正在重试 ({attempt}/{max_attempts})...', 'en': 'Proxy test failed, retrying ({attempt}/{max_attempts})...'}, 'proxy_test_backup_url': {'zh': '主测试URL失败，尝试备用URL...', 'en': 'Primary test URL failed, trying backup URL...'}, 'proxy_test_all_failed': {'zh': '所有代理测试尝试均失败', 'en': 'All proxy test attempts failed'}, 'querying_ports_status': {'zh': '正在查询端口: {ports_desc}...', 'en': 'Querying ports: {ports_desc}...'}, 'port_query_result_format': {'zh': '[{port_type} - {port_num}] {pid_info}', 'en': '[{port_type} - {port_num}] {pid_info}'}, 'port_not_in_use_format': {'zh': '[{port_type} - {port_num}] 未被占用', 'en': '[{port_type} - {port_num}] Not in use'}, 'pids_on_multiple_ports_label': {'zh': '多端口占用情况:', 'en': 'Multi-Port Usage:'}, 'launch_llm_service_btn': {'zh': '启动本地LLM模拟服务', 'en': 'Launch Local LLM Mock Service'}, 'stop_llm_service_btn': {'zh': '停止本地LLM模拟服务', 'en': 'Stop Local LLM Mock Service'}, 'llm_service_name_key': {'zh': '本地LLM模拟服务', 'en': 'Local LLM Mock Service'}, 'status_llm_starting': {'zh': '本地LLM模拟服务启动中 (PID: {pid})...', 'en': 'Local LLM Mock Service starting (PID: {pid})...'}, 'status_llm_stopped': {'zh': '本地LLM模拟服务已停止。', 'en': 'Local LLM Mock Service stopped.'}, 'status_llm_stop_error': {'zh': '停止本地LLM模拟服务时出错。', 'en': 'Error stopping Local LLM Mock Service.'}, 'status_llm_already_running': {'zh': '本地LLM模拟服务已在运行 (PID: {pid})。', 'en': 'Local LLM Mock Service is already running (PID: {pid}).'}, 'status_llm_not_running': {'zh': '本地LLM模拟服务未在运行。', 'en': 'Local LLM Mock Service is not running.'}, 'status_llm_backend_check': {'zh': '正在检查LLM后端服务 ...', 'en': 'Checking LLM backend service ...'}, 'status_llm_backend_ok_starting': {'zh': 'LLM后端服务 (localhost:{port}) 正常，正在启动模拟服务...', 'en': 'LLM backend service (localhost:{port}) OK, starting mock service...'}, 'status_llm_backend_fail': {'zh': 'LLM后端服务 (localhost:{port}) 未响应，无法启动模拟服务。', 'en': 'LLM backend service (localhost:{port}) not responding, cannot start mock service.'}, 'confirm_stop_llm_title': {'zh': '确认停止LLM服务', 'en': 'Confirm Stop LLM Service'}, 'confirm_stop_llm_message': {'zh': '确定要停止本地LLM模拟服务吗?', 'en': 'Are you sure you want to stop the Local LLM Mock Service?'}}
current_language = 'zh'
...
proxy_address_var: Optional[tk.StringVar] = None
proxy_enabled_var: Optional[tk.BooleanVar] = None
active_auth_file_display_var: Optional[tk.StringVar] = None
LLM_PY_FILENAME = 'llm.py'
llm_service_process_info: Dict[str, Any] = {'popen': None, 'monitor_thread': None, 'stdout_thread': None, 'stderr_thread': None, 'service_name_key': 'llm_service_name_key'}
...
def kill_process_pid(pid: int) -> bool:
    ...
    try:
        if system == 'Linux' or system == 'Darwin':
            ...
            subprocess.run(['kill', '-TERM', str(pid)], capture_output=True, text=True, timeout=3)
            time.sleep(0.5)
            try:
                ...
                subprocess.run(['kill', '-KILL', str(pid)], check=True, capture_output=True, text=True, timeout=3)
                time.sleep(0.1)
...
def check_all_required_ports(ports_to_check: List[Tuple[int, str]]) -> bool:
    ...
    if messagebox.askyesno(get_text('confirm_kill_multiple_ports_title'), get_text('confirm_kill_multiple_ports_message', occupied_ports_details=details_str), parent=root_widget):
            ...
            if info['pids_data']:
                for p_data in info['pids_data']:
                    ...
                    if normal_kill_ok:
                        ...
                        if messagebox.askyesno(get_text('confirm_stop_pid_admin_title'), get_text('confirm_stop_pid_admin_message', pid=pid, name=name), parent=root_widget):
                            logger.info(f'Port Check Cleanup: User approved admin kill for PID {pid} ({name}). Attempting.')
                            admin_kill_initiated = kill_process_pid_admin(pid)
...
def open_webui_gui():
    port = get_fastapi_port_from_gui()
...
def manage_auth_files_gui():
    if not os.path.exists(AUTH_PROFILES_DIR):
...
def is_service_running():
    return managed_process_info.get('popen') and managed_process_info['popen'].poll() is None and (not managed_process_info.get('fully_detached', False))
    ...
    if managed_process_info.get('service_name_key') == service_name_key:
        ...
        if not is_detached:
            if exit_code == 0:
                ...
                update_status_bar('service_stopped_exit_code_status', service_name=service_name, code=exit_code)
...
DEFAULT_CONFIG = {'fastapi_port': DEFAULT_FASTAPI_PORT, 'camoufox_debug_port': DEFAULT_CAMOUFOX_PORT_GUI, 'stream_port': int(os.environ.get('GUI_DEFAULT_STREAM_PORT', '3120')), 'stream_port_enabled': True, 'helper_endpoint': os.environ.get('GUI_DEFAULT_HELPER_ENDPOINT', ''), 'helper_enabled': False, 'proxy_address': os.environ.get('GUI_DEFAULT_PROXY_ADDRESS', 'http://127.0.0.1:7890'), 'proxy_enabled': False}
...
def save_config():
    config = {'fastapi_port': port_entry_var.get(), 'camoufox_debug_port': camoufox_debug_port_var.get(), 'stream_port': stream_port_var.get(), 'stream_port_enabled': stream_port_enabled_var.get(), 'helper_endpoint': helper_endpoint_var.get(), 'helper_enabled': helper_enabled_var.get(), 'proxy_address': proxy_address_var.get(), 'proxy_enabled': proxy_enabled_var.get()}
    try:
...
def _launch_process_gui(cmd: List[str], service_name_key: str, env_vars: Optional[Dict[str, str]]=None):
    ...
    if system == 'Windows':
        ...
        shell_command_parts = [f'cd {script_dir_quoted}', '&&']
        if env_prefix_str:
        ...
        if terminal_emulator:
            ...
            elif 'xterm' in terminal_emulator:
        ...
        update_status_bar('status_error_starting', service_name=service_name)
        return
    if not launch_cmd_for_terminal:
...
def start_headed_interactive_gui():
    ...
    if port_auto_check_var.get():
        ports_to_check = [(launch_params['fastapi_port'], 'fastapi'), (launch_params['camoufox_debug_port'], 'camoufox_debug')]
    ...
    cmd = build_launch_command('debug', launch_params['fastapi_port'], launch_params['camoufox_debug_port'], launch_params['stream_port_enabled'], launch_params['stream_port'], launch_params['helper_enabled'], launch_params['helper_endpoint'])
...
def start_headless_gui():
    ...
    if port_auto_check_var.get():
        ...
        if not check_all_required_ports(ports_to_check):
            return
    proxy_env = _configure_proxy_env_vars()
    cmd = build_launch_command('headless', launch_params['fastapi_port'], launch_params['camoufox_debug_port'], launch_params['stream_port_enabled'], launch_params['stream_port'], launch_params['helper_enabled'], launch_params['helper_endpoint'])
...
def start_virtual_display_gui():
    ...
    if port_auto_check_var.get():
        ...
        if not check_all_required_ports(ports_to_check):
            return
    proxy_env = _configure_proxy_env_vars()
    cmd = build_launch_command('virtual-display', launch_params['fastapi_port'], launch_params['camoufox_debug_port'], launch_params['stream_port_enabled'], launch_params['stream_port'], launch_params['helper_enabled'], launch_params['helper_endpoint'])
...
def is_llm_service_running() -> bool:
    return llm_service_process_info.get('popen') and llm_service_process_info['popen'].poll() is None

def monitor_llm_process_thread_target():
    popen = llm_service_process_info.get('popen')
    service_name_key = llm_service_process_info.get('service_name_key')
    ...
    if popen.stdout:
        llm_service_process_info['stdout_thread'] = threading.Thread(target=enqueue_stream_output, args=(popen.stdout, f'{service_name}-stdout'), daemon=True)
        llm_service_process_info['stdout_thread'].start()
    if popen.stderr:
        llm_service_process_info['stderr_thread'] = threading.Thread(target=enqueue_stream_output, args=(popen.stderr, f'{service_name}-stderr'), daemon=True)
...
def _actually_launch_llm_service():
    ...
    if not os.path.exists(llm_script_path):
        ...
        update_status_bar('status_script_not_found', service_name=service_name)
        return
    main_server_port = get_fastapi_port_from_gui()
    ...
    try:
        ...
        effective_env['PYTHONIOENCODING'] = 'utf-8'
        popen = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=False, cwd=SCRIPT_DIR, env=effective_env, creationflags=subprocess.CREATE_NO_WINDOW if platform.system() == 'Windows' else 0)
...
def _check_llm_backend_and_launch_thread():
    ...
    if root_widget:
        if backend_ok:
            update_status_bar(backend_ok_msg_key, port=current_fastapi_port)
            _actually_launch_llm_service()
        else:
            update_status_bar(backend_fail_msg_key, port=current_fastapi_port)
            messagebox.showwarning(get_text('warning_title'), get_text(backend_fail_msg_key, port=current_fastapi_port), parent=root_widget)
...
def query_port_and_display_pids_gui():
    ...
    if stream_port_enabled_var.get():
        try:
            ...
            stream_p = int(stream_p_val_str) if stream_p_val_str else 0
    ...
    if pid_listbox_widget and pid_list_lbl_frame_ref:
        ...
        pid_list_lbl_frame_ref.config(text=get_text('pids_on_multiple_ports_label'))
        ...
        for port_info in ports_to_query_info:
            ...
            if processes_on_current_port:
                ...
                for proc_info in processes_on_current_port:
                    ...
                    display_text = get_text('port_query_result_format', port_type=port_type_name, port_num=current_port, pid_info=pid_display_info)
                    pid_listbox_widget.insert(tk.END, display_text)
            else:
                display_text = get_text('port_not_in_use_format', port_type=port_type_name, port_num=current_port)
                pid_listbox_widget.insert(tk.END, display_text)
        if not found_any_process and (not any((find_processes_on_port(p['port']) for p in ports_to_query_info))):
            pass
    else:
...
def _perform_proxy_test(proxy_address: str, test_url: str) -> Tuple[bool, str]:
    ...
    for url_index, current_url in enumerate(urls_to_try):
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            should_retry = status_code == 503 or 'timeout' in error_msg.lower() or 'temporarily unavailable' in error_msg.lower()
            if not should_retry:
    ...
    try:
        ...
        def update_gui():
            if success:
                ...
                messagebox.showerror(get_text('error_title'), get_text('proxy_test_failure', url=test_url, error=message), parent=root_widget)
        ...
        def show_error():
            messagebox.showerror(get_text('error_title'), f'代理测试过程中发生错误: {e}', parent=root_widget)
...
def test_proxy_connectivity_gui():
    ...
    test_url = LANG_TEXTS['proxy_test_url_default']
    update_status_bar('proxy_testing_status', proxy_addr=proxy_addr_to_test)
    test_thread = threading.Thread(target=_proxy_test_thread, args=(proxy_addr_to_test, test_url), daemon=True)
    test_thread.start()
...
def stop_selected_pid_from_list_gui():
    ...
    try:
        ...
        is_no_process_entry = no_process_indicator_zh in selected_text or no_process_indicator_en in selected_text or selected_text == general_no_pids_msg_zh or (selected_text == general_no_pids_msg_en)
        if is_no_process_entry:
            logger.info(f"Selected item is a 'no process' entry: {selected_text}")
            return
        match = re.match('^(?:\\[[^\\]]+\\]\\s*)?(\\d+)\\s*-\\s*(.*)$', selected_text)
        if match:
    ...
    if messagebox.askyesno(get_text('confirm_stop_pid_title'), get_text('confirm_stop_pid_message', pid=pid_to_stop, name=process_name_to_stop), parent=root_widget):
        ...
        elif messagebox.askyesno(get_text('confirm_stop_pid_admin_title'), get_text('confirm_stop_pid_admin_message', pid=pid_to_stop, name=process_name_to_stop), parent=root_widget):
    ...
    try:
        if system == 'Windows':
            ...
            if ctypes.windll.shell32.IsUserAnAdmin() == 0:
                ...
                result = subprocess.run(['taskkill', '/PID', str(pid), '/F', '/T'], capture_output=True, text=True, creationflags=subprocess.CREATE_NO_WINDOW)
...
def kill_custom_pid_gui():
    ...
    if messagebox.askyesno(get_text('confirm_kill_custom_pid_title'), confirm_msg, parent=root_widget):
        ...
        elif messagebox.askyesno(get_text('confirm_stop_pid_admin_title'), get_text('confirm_stop_pid_admin_message', pid=pid_to_kill, name=process_name_to_kill), parent=root_widget):
...
def build_gui(root: tk.Tk):
    ...
    global active_auth_file_display_var
    ...
    active_auth_file_display_var = tk.StringVar()
    ...
    main_paned_window.add(left_frame_container, weight=3)
    left_frame_container.columnconfigure(0, weight=1)
    left_frame_container.rowconfigure(0, weight=0)
    left_frame_container.rowconfigure(1, weight=0)
    left_frame_container.rowconfigure(2, weight=0)
    left_frame_container.rowconfigure(3, weight=1)
    left_current_row = 0
    ...
    left_current_row += 1
    lbl_launch_options_note = ttk.Label(launch_options_frame, text='', wraplength=240)
    ...
    btn_headless = ttk.Button(launch_options_frame, text='', command=start_headless_gui)
    btn_headless.pack(fill=tk.X, padx=5, pady=3)
    widgets_to_translate.append({'widget': btn_headless, 'key': 'launch_headless_btn'})
    ...
    btn_open_webui = ttk.Button(launch_options_frame, text='', command=open_webui_gui)
    btn_open_webui.pack(fill=tk.X, padx=5, pady=3)
    widgets_to_translate.append({'widget': btn_open_webui, 'key': 'open_webui_btn'})
    ttk.Separator(launch_options_frame, orient=tk.HORIZONTAL).pack(fill=tk.X, padx=5, pady=(8, 5))
    ...
    left_frame_container.rowconfigure(left_current_row, weight=1)
    middle_frame_container = ttk.Frame(main_paned_window, padding='5')
    main_paned_window.add(middle_frame_container, weight=2)
    ...
    middle_frame_container.rowconfigure(2, weight=0)
    ...
    main_paned_window.add(right_frame_container, weight=2)
    ...
    output_scrolled_text = scrolledtext.ScrolledText(output_log_area_frame, height=10, width=35, wrap=tk.WORD, state=tk.DISABLED)
...
def show_service_closing_guide():
    messagebox.showinfo(get_text('service_closing_guide'), get_text('service_closing_guide_message'), parent=root_widget)
