...
本文档详细介绍如何使�?`.env` 文件来配�?AI Studio Proxy API 项目，实现统一的配置管理�?
...
- �?**版本更新无忧**: 一�?`git pull` 就完成更新，无需重新配置
- �?**配置集中管理**: 所有配置项统一�?`.env` 文件中，清晰明了
- �?**启动命令简�?*: 无需复杂的命令行参数，一键启�?
- �?**安全�?*: `.env` 文件已被 `.gitignore` 忽略，不会泄露敏感配�?
- �?**灵活�?*: 支持不同环境的配置管理（开发、测试、生产）
- �?**Docker 兼容**: Docker 和本地环境使用相同的配置方式
- �?**模块化设�?*: 配置项按功能分组，便于理解和维护
...
**就这么简单！** 无需复杂的命令行参数，所有配置都�?`.env` 文件中预设好了�?
...
# 是否在处理请求时自动打开并使�?"URL Context" 功能
# 参�? https://ai.google.dev/gemini-api/docs/url-context
ENABLE_URL_CONTEXT=true

# 是否默认启用 "指定思考预�? 功能 (true/false),不启用时模型一般将自行决定思考预�?
# �?API 请求中未提供 reasoning_effort 参数�?将使用此值�?
...
# �?API 请求中未提供 tools 参数时，将使用此设置作为 Google Search 的默认开关状态�?
...
USERSCRIPT_PATH=browser/more_models.js
```

**脚本注入功能 v3.0 重大升级**�?
- **🚀 Playwright 原生拦截**: 使用 Playwright 路由拦截�?00% 可靠�?
- **🔄 双重保障机制**: 网络拦截 + 脚本注入，确保万无一�?
- **📝 直接脚本解析**: 从油猴脚本中自动解析模型列表，无需配置文件
- **🔗 前后端同�?*: 前端和后端使用相同的模型数据�?
- **⚙️ 零配置维�?*: 脚本更新时自动获取新的模型列�?
- **🔄 自动适配**: 油猴脚本更新时无需手动更新配置

**�?v1.x 的主要区�?*�?
- 移除�?`MODEL_CONFIG_PATH` 配置项（已废弃）
- 不再需要手动维护模型配置文�?
- 工作机制�?配置文件 + 脚本注入"改为"直接脚本解析 + 网络拦截"

详细使用方法请参�?[脚本注入指南](script_injection_guide.md)�?
...
# 使用自定义脚本（模型数据直接从脚本解析）
USERSCRIPT_PATH=browser/my_custom_script.js
...
- `.env` 文件已被 `.gitignore` 忽略，不会被提交到版本控�?
- 请勿�?`.env.example` 中包含真实的敏感信息
- 如需分享配置，请复制并清理敏感信息后再分�?
