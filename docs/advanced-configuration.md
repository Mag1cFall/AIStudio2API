...
1. **`--internal-camoufox-proxy` 命令行参�?* (最高优先级)
...
此代理配置会同时应用�?Camoufox 浏览器和流式代理服务的上游连接，确保整个系统的代理行为一致�?
...
在此模式下，主服务器会优先尝试通过端口 `3120` (或指定的 `--stream-port`) 上的集成流式代理获取响应。如果失败，则回退�?Playwright 页面交互�?
...
在此模式下，主服务器会优先尝试通过 `--helper` 指定的端点获取响�?(需要有效的 `auth_profiles/active/*.json` 以提�?`SAPISID`)。如果失败，则回退�?Playwright 页面交互�?

### 模式3: 仅使�?Playwright 页面交互 (禁用所有流式代理和 Helper)
...
在此模式下，主服务器将仅通过 Playwright �?AI Studio 页面交互 (模拟点击"编辑"�?复制"按钮) 来获取响应。这是传统的后备方法�?

## 虚拟显示模式 (Linux)

### 关于 `--virtual-display`

- **为什么使�?*: 与标准的无头模式相比，虚拟显示模式通过创建一个完整的虚拟 X 服务器环�?(Xvfb) 来运行浏览器。这可以模拟一个更真实的桌面环境，从而可能进一步降低被网站检测为自动化脚本或机器人的风险
- **什么时候使�?*: 当您�?Linux 环境下运行，并且希望以无头模式操�?
- **如何使用**:
  1. 确保您的 Linux 系统已安�?`xvfb`
...
- 创建一�?HTTP 代理服务器（默认端口�?120�?
- 拦截针对 Google 域名�?HTTPS 请求
- 使用自签�?CA 证书动态自动生成服务器证书
- �?AIStudio 响应解析�?OpenAI 兼容格式

## 模型排除配置

### excluded_models.txt

项目根目录下�?`excluded_models.txt` 文件可用于从 `/v1/models` 端点返回的列表中排除特定的模�?ID�?
...
1. **双重拦截机制**�?
   - **Playwright 路由拦截**：在网络层面直接拦截和修改模型列表响�?
   - **JavaScript 脚本注入**：作为备用方案，确保万无一�?
...
您的自定义脚本必须包�?`MODELS_TO_INJECT` 数组�?
...
1. **请求识别**：检测包�?`alkalimakersuite` �?`ListModels` 的请�?
2. **响应获取**：获取原始模型列表响�?
3. **数据解析**：解�?JSON 响应并处理反劫持前缀
4. **模型注入**：将自定义模型注入到响应�?
5. **响应返回**：返回修改后的响应给浏览�?
...
1. **检查脚本文�?*�?
   ...
   cat browser/more_models.js | head -20
   ```
...
1. **前端检�?*：在浏览器开发者工具中查看是否�?JavaScript 错误
2. **后端检�?*：查�?API 响应是否包含注入的模�?
3. **网络检�?*：确认网络拦截是否正常工�?
...
- 网络拦截仅限于特定的模型列表请求
- 不会拦截或修改其他敏感请�?
- 所有修改都在本地进行，不会发送到外部服务�?

## GUI 启动器高级功�?

新的 `app_launcher.py` 提供了一个基�?Web 的现代化管理界面�?

### 仪表�?(Dashboard)
- 实时查看服务状�?(Running/Stopped/Starting)
- 查看服务 PID
- 一键启�?停止服务
- 实时查看详细日志，支持过�?(INFO/WARN/ERROR)
...
**强烈建议在所�?`launch_camoufox.py` 命令中明确指�?`--internal-camoufox-proxy` 参数，即使其值为空字符串 (`''`)，以避免意外使用系统环境变量中的代理设置�?*

### 参数控制限制

API 请求中的模型参数（如 `temperature`, `max_output_tokens`, `top_p`, `stop`�?*仅在通过 Playwright 页面交互获取响应时生�?*。当使用集成的流式代理或外部 Helper 服务时，这些参数的传递和应用方式取决于这些服务自身的实现�?

### 首次访问性能

当通过流式代理首次访问一个新�?HTTPS 主机时，服务需要为该主机动态生成并签署一个新的子证书。这个过程可能会比较耗时，导致对该新主机的首次连接请求响应较慢。一旦证书生成并缓存后，后续访问同一主机将会显著加快�?

## 下一�?

高级配置完成后，请参考：
- [脚本注入指南](script_injection_guide.md) - 详细的脚本注入功能使用说�?
