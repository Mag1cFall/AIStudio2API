...
func decodeInlineMedia(raw json.RawMessage, path string, evidence json.RawMessage) (Media, error) {
	...
	if len(values) < 2 {
		return Media{}, &ProtocolEvidenceError{Method: "GenerateContent", Path: path, Detail: "insufficient inline media fields", Raw: raw}
	}
	...
	if err != nil {
		return Media{}, &ProtocolEvidenceError{Method: "GenerateContent", Path: path + "[1]", Detail: "inline media is not valid Base64", Raw: raw}
	}
